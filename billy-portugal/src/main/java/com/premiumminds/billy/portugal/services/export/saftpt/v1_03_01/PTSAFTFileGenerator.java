...
public class PTSAFTFileGenerator {
  ...
  @Inject
  public PTSAFTFileGenerator(DAOPTCustomer daoCustomer, DAOPTSupplier daoSupplier,
      DAOPTProduct daoProduct, DAOPTTax daoPTTax, DAOPTRegionContext daoPTRegionContext,
      DAOPTInvoice daoPTInvoice, DAOPTSimpleInvoice daoPTSimpleInvoice,
      DAOPTReceiptInvoice daoPTReceiptInvoice, DAOPTCreditNote daoPTCreditNote) {
    ...
    try {
      this.jaxbContext = JAXBContext
          .newInstance("com.premiumminds.billy.portugal.services.export.saftpt.v1_03_01.schema");
  ...
  public AuditFile generateSAFTFile(final OutputStream targetStream,
      final PTBusinessEntity businessEntity, final PTApplicationEntity application,
      final String certificateNumber, final Date fromDate, final Date toDate)
      throws SAFTPTExportException {
  ...
  private Header generateHeader(PTBusinessEntity businessEntity, PTApplicationEntity application,
      String certificateNumber, Date startDate, Date endDate) throws DatatypeConfigurationException,
    ...
    hdr.setCompanyID(this.validateString("CompanyID", businessEntity.getFinancialID(),
        this.MAX_LENGTH_50, true));
    hdr.setTaxRegistrationNumber(this.validateInteger("TaxRegistrationNumber",
        businessEntity.getFinancialID(), this.MAX_LENGTH_9, true));
    hdr.setTaxAccountingBasis(
        this.validateString("TaxAccountBasis", this.TAX_ACCOUNTING_BASIS, this.MAX_LENGTH_1, true));
    hdr.setCompanyName(
        this.validateString("CompanyName", businessEntity.getName(), this.MAX_LENGTH_100, true));

    if ((this.optionalParam = this.validateString("BusinessName",
        businessEntity.getCommercialName(), this.MAX_LENGTH_60, false)).length() > 0) {
      hdr.setBusinessName(this.optionalParam);
    }

    hdr.setCompanyAddress(
        this.generateAddressStructurePT((AddressEntity) businessEntity.getAddress()));
    ...
    hdr.setCurrencyCode(
        this.validateString("CurrencyCode", this.CURRENCY_CODE, this.MAX_LENGTH_3, true));
    ...
    hdr.setSoftwareCertificateNumber(this.validateBigInteger("SoftwareCertificateNumber",
        certificateNumber, this.MAX_LENGTH_255, true));
    hdr.setProductID(this.validateString("ProductID",
        application.getName() + "/" + application.getDeveloperCompanyName(), this.MAX_LENGTH_255,
        true));
  ...
  private Customer generateCustomer(PTCustomerEntity customerEntity)
    ...
    customer
        .setAccountID(this.validateString("AccountID", this.ACCOUNT_ID, this.MAX_LENGTH_30, true));
    customer.setSelfBillingIndicator(this.validateInteger("SelfBillingIndicator",
        this.SELF_BILLING_INDICATOR, this.MAX_LENGTH_1, true));
  ...
  private Supplier generateSupplier(PTSupplierEntity supplierEntity)
    ...
    supplier.setSupplierID(this.validateString("SupplierID", supplierEntity.getID().toString(),
    ...
    supplier.setSupplierTaxID(this.validateString("SupplierTaxID",
        supplierEntity.getTaxRegistrationNumber(), this.MAX_LENGTH_20, true));

    supplier.setCompanyName(
        this.validateString("CompanyName", supplierEntity.getName(), this.MAX_LENGTH_100, true));

    if ((this.optionalParam = this.validateString("Contact", supplierEntity.getReferralName(),
        this.MAX_LENGTH_50, false)).length() > 0) {
      supplier.setContact(this.optionalParam);
    }

    supplier.setBillingAddress(this
        .generateSupplierAddressStructure((PTAddressEntity) supplierEntity.getBillingAddress()));

    if (supplierEntity.getShippingAddress() != null) {
      supplier.getShipFromAddress().add(this
          .generateSupplierAddressStructure((PTAddressEntity) supplierEntity.getShippingAddress()));
    }

    supplier.setSelfBillingIndicator(this.validateInteger("SelfBillingIndicator",
        this.SELF_BILLING_INDICATOR, this.MAX_LENGTH_1, true));
  ...
  private Product generateProduct(PTProductEntity productEntity)
    ...
    product.setProductType(this.validateString("ProductType",
        this.getProductType(productEntity.getType()), this.MAX_LENGTH_1, true));

    product.setProductCode(this.validateString("ProductCode", productEntity.getID().toString(),
        this.MAX_LENGTH_30, true));

    product.setProductGroup(this.validateString("ProductGroup", productEntity.getProductGroup(),
    ...
    if (productEntity.getNumberCode() == null || productEntity.getNumberCode().length() == 0) {
      ...
      product.setProductNumberCode(this.validateString("ProductCode", productEntity.getNumberCode(),
          this.MAX_LENGTH_50, true));
    }
  ...
  private TaxTable generateTaxTable(List<PTTaxEntity> taxEntities)
    ...
    for (PTTaxEntity taxEntity : taxEntities) {
      ...
      tte.setTaxType(
          this.validateString("TaxType", this.getTaxType(taxEntity), this.MAX_LENGTH_3, true));
      ...
      tte.setDescription(this.validateString("Description", taxEntity.getDescription(),
  ...
  private SourceDocuments generateSourceDocuments(List<PTInvoiceEntity> invoices,
    ...
    salesInvoices.setNumberOfEntries(new BigInteger(Integer.toString(
        invoices.size() + simpleInvoices.size() + receiptInvoices.size() + creditNotes.size())));
  ...
  /**
   * Generates a financial document that will be inserted in the SourceDocuments
   * table
   * 
   * @param document
   *          - can be an invoice, simple invoice or credit note
   * @param documentType
   * @return an instance of Invoice (represents a financial document in the SAFT
   *         XML context)
   * 
   * @throws DatatypeConfigurationException
   * @throws RequiredFieldNotFoundException
  ...
  private Invoice generateSAFTInvoice(PTGenericInvoiceEntity document)
    ...
    saftInv.setInvoiceType(
        validateString("InvoiceType", getDocumentType(document), MAX_LENGTH_2, true));
    ...
    if (document.getHashControl() != null) {
      saftInv.setHashControl(
          validateString("HashControl", document.getHashControl(), MAX_LENGTH_40, false));
    }
    saftInv.setPeriod(validateInteger("Period",
        Integer.toString(getDateField(document.getDate(), Calendar.MONTH) + 1), MAX_LENGTH_2,
        true));
    saftInv.setInvoiceDate(formatDate(document.getDate()));
    saftInv.setSpecialRegimes(getSpecialRegimes(document.isSelfBilled(),
        document.isCashVATEndorser(), document.isThirdPartyBilled()));
    saftInv.setSourceID(
        validateString("InvoiceSourceID", document.getSourceId(), MAX_LENGTH_30, true));
    ...
    if (document.getShippingDestination() != null) {
      saftInv.setShipTo(
          this.getShippingPointStructure(document.getShippingDestination().getDeliveryId(),
  ...
  private SpecialRegimes getSpecialRegimes(Boolean selfBilled, Boolean cashVATEndorser,
      Boolean thirdPartyBilled) throws RequiredFieldNotFoundException {
    SpecialRegimes rvalue = new SpecialRegimes();
    if (null != selfBilled) {
      rvalue.setSelfBillingIndicator(
          validateInteger("SelfBillingIndicator", selfBilled ? "1" : "0", MAX_LENGTH_1, true));
    } else {
      rvalue.setSelfBillingIndicator(0);
    }
    if (null != cashVATEndorser) {
      rvalue.setCashVATSchemeIndicator(validateInteger("CashVATSchemeIndicator",
          cashVATEndorser ? "1" : "0", MAX_LENGTH_1, true));
    } else {
      rvalue.setCashVATSchemeIndicator(0);
    }
    if (null != thirdPartyBilled) {
      rvalue.setThirdPartiesBillingIndicator(validateInteger("ThirdPartiesBillingIndicator",
   ...
   * @throws InvalidPaymentMechanismException
   */
  private void processDocument(Invoice saftInvoice, PTGenericInvoiceEntity document,
      boolean isCredit) throws RequiredFieldNotFoundException, DatatypeConfigurationException,
      InvalidDocumentTypeException, InvalidTaxTypeException, InvalidTaxCodeException,
    ...
    for (PTGenericInvoiceEntryEntity entry : entries) {
      ...
      line.setProductCode(this.validateString("ProductCode", entry.getProduct().getID().toString(),
          this.MAX_LENGTH_30, true));
      line.setProductDescription(this.validateString("ProductDescription",
          entry.getProduct().getDescription(), this.MAX_LENGTH_200, true));
      line.setQuantity(this.validateBigDecimal(entry.getQuantity()));
      line.setUnitOfMeasure(
          this.validateString("UnitOfMeasure", this.UNIT_OF_MEASURE, this.MAX_LENGTH_20, true));
      ...
      /* REQUIRED */
      line.setDescription(
          this.validateString("Description", entry.getDescription(), this.MAX_LENGTH_200, true));
      if (isCredit) {
      ...
      if (entry.getTaxes().size() > 0) {
        ...
        tax.setTaxType(
            this.validateString("TaxType", this.getTaxType(taxEntity), this.MAX_LENGTH_3, true));
        tax.setTaxCode(
            this.validateString("TaxCode", taxEntity.getCode(), this.MAX_LENGTH_10, true));
        tax.setTaxCountryRegion(this.validateString("TaxCountryRegion",
            this.getRegionCodeFromISOCode(
                ((PTRegionContext) taxEntity.getContext()).getRegionCode()),
            this.MAX_LENGTH_5, true));
        ...
        if ((tax.getTaxPercentage() != null && tax.getTaxPercentage().equals(BigDecimal.ZERO))
          ...
          line.setTaxExemptionReason(this.validateString("TaxExemptionReason",
              entry.getTaxExemptionReason(), this.MAX_LENGTH_60, true));
        }
      }
   ...
   * @throws InvalidDocumentTypeException
   */
  private References getReferencesForDocumentEntry(PTGenericInvoiceEntryEntity entry,
      PTGenericInvoiceEntity document) throws RequiredFieldNotFoundException,
    ...
    if (referencedDocument != null) {
      ...
      if ((this.optionalParam = this.validateString("References.CreditNote.Reason", reasonForCredit,
          this.MAX_LENGTH_50, false)).length() > 0) {
  ...
  /**
   * Constructs an instance of ShippingPointStructure needed in the construction
   * of a SAFT Invoice
   * 
   * @param deliveryID
  ...
  private ShippingPointStructure getShippingPointStructure(String deliveryID, Date deliveryDate,
    ...
    if ((this.optionalParam = this.validateString("DeliveryID", deliveryID, this.MAX_LENGTH_30,
        false)).length() > 0) {
  ...
  private Settlement getSettlement(PTGenericInvoiceEntity document)
    ...
    if (document.getSettlementDiscount() != null) {
      ...
      if ((this.optionalParam = this.validateString("SettlementDiscount",
          document.getSettlementDescription(), this.MAX_LENGTH_30, false)).length() > 0) {
  ...
  private DocumentTotals getDocumentTotals(PTGenericInvoiceEntity document, boolean isCredit)
    ...
    if (!this.validateBigDecimal(document.getAmountWithoutTax())
        .equals(this.validateBigDecimal(BigDecimal.ZERO))) {
   ...
   * @throws RequiredFieldNotFoundException
   */
  private void updateCustomerGeneralInfo(Customer customer, String customerID,
      String customerFinancialID, String companyName, PTAddressEntity address)
      throws RequiredFieldNotFoundException {
    customer.setCustomerTaxID(
        this.validateString("CustomerTaxID", customerFinancialID, this.MAX_LENGTH_20, true));

    customer
        .setCompanyName(this.validateString("CompanyName", companyName, this.MAX_LENGTH_100, true));
  ...
  private List<PaymentMethod> getPaymentsList(PTGenericInvoiceEntity document)
      throws RequiredFieldNotFoundException, InvalidPaymentMechanismException,
      DatatypeConfigurationException {
    ...
    if (document.getPayments() != null) {
      ...
      for (com.premiumminds.billy.core.services.entities.Payment p : document.getPayments()) {
        ...
        payment.setPaymentMechanism(this.validateString("PaymentMechanism",
            this.getPaymentMechanism((Enum<PaymentMechanism>) p.getPaymentMethod()),
   ...
   ***********************/
  /**
   * Constructs an AddressStructures that is used in almost every fields of SAFT
   * file that represent addresses
   * 
   * @param addressEntity
   * @return
   * @throws RequiredFieldNotFoundException
  ...
  private AddressStructure generateAddressStructure(PTAddressEntity addressEntity)
    ...
    if ((this.optionalParam = this.validateString("BuildingNumber", addressEntity.getNumber(),
        this.MAX_LENGTH_10, false)).length() > 0) {
      address.setBuildingNumber(this.optionalParam);
    }

    if ((this.optionalParam = this.validateString("StreetName", addressEntity.getStreetName(),
        this.MAX_LENGTH_90, false)).length() > 0) {
      address.setStreetName(this.optionalParam);
    }

    address.setAddressDetail(this.validateString("AddressDetail", addressEntity.getDetails(),
    ...
    if ((this.optionalParam = this.validateString("Region", addressEntity.getRegion(),
        this.MAX_LENGTH_50, false)).length() > 0) {
      address.setRegion(this.optionalParam);
    }

    address.setCountry(
        this.validateString("Country", addressEntity.getISOCountry(), this.MAX_LENGTH_12, true));

    return address;
  }

  /**
   * Constructs a specific AddressStructurePT that is only used in the Header of
   * the SAFT File
   * 
   * @param addressEntity
   * @return
  ...
  /**
   * Constructs an SupplierAddressStructures that is used in the Supplier of the
   * SAFT file
   * 
   * @param addressEntity
   * @return
  ...
  private SupplierAddressStructure generateSupplierAddressStructure(PTAddressEntity addressEntity)
    ...
    address.setCountry(
        this.validateString("Country", addressEntity.getISOCountry(), this.MAX_LENGTH_2, true));
  ...
  private void setContacts(Header hdr, List<PTContactEntity> contacts)
    ...
    for (ContactEntity ce : contacts) {
      ...
      if ((this.optionalParam = this.validateString("Telephone", ce.getTelephone(),
          this.MAX_LENGTH_20, false)).length() > 0) {
        hdr.setTelephone(this.optionalParam);
      } else {
        if ((this.optionalParam = this.validateString("Telephone", ce.getMobile(),
            this.MAX_LENGTH_20, false)).length() > 0) {
          hdr.setTelephone(this.optionalParam);
        }
      }
      ...
      if ((this.optionalParam = this.validateString("Email", ce.getEmail(), this.MAX_LENGTH_60,
          false)).length() > 0) {
        hdr.setEmail(this.optionalParam);
      }

      if ((this.optionalParam = this.validateString("Website", ce.getWebsite(), this.MAX_LENGTH_60,
          false)).length() > 0) {
  ...
  /**
   * Validates an integer, considering the max length of that field that will be
   * assigned and if that field is required
   * 
   * @param field
   * @param str
  ...
  /**
   * Validates a BigInteger, considering the max length of that field that will
