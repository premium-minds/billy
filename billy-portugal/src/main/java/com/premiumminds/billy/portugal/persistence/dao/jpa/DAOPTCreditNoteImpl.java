...
public class DAOPTCreditNoteImpl extends DAOPTGenericInvoiceImpl implements DAOPTCreditNote {
  ...
  public List<PTCreditNoteEntity> getBusinessCreditNotesForSAFTPT(UID uid, Date from, Date to) {
    ...
    query.from(creditNote)
        .where(creditNote.instanceOf(JPAPTCreditNoteEntity.class)
            .and(creditNote.date.between(from, to)).and(
                toDSL(creditNote.business, QJPAPTBusinessEntity.class).uid.eq(uid.toString())));
  ...
  public List<PTCreditNote> findByReferencedDocument(UID uidCompany, UID uidInvoice) {
    ...
    JPASubQuery entQ = new JPASubQuery().from(entry).where(
        toDSL(entry.reference, QJPAPTGenericInvoiceEntity.class).uid.in(invQ.list(invoice.uid)));

    return (List<PTCreditNote>) (List<?>) createQuery().from(creditNote)
        .where(toDSL(creditNote.business, QJPAPTBusinessEntity.class).uid.eq(uidCompany.toString())
            .and(toDSL(creditNote.entries.any(), QJPAPTCreditNoteEntryEntity.class).uid
