...
public class PTReceiptInvoicePDFExportHandler extends AbstractPDFExportHandler {
	...
	private DAOPTReceiptInvoice daoPTReceiptInvoice;
	private Config config;

	@Inject
	public PTReceiptInvoicePDFExportHandler(DAOPTReceiptInvoice daoPTReceiptInvoice) {
		super(daoPTReceiptInvoice);
		this.daoPTReceiptInvoice = daoPTReceiptInvoice;
		this.config = new Config();
	}

	public File toFile(URI fileURI, PTReceiptInvoiceEntity invoice,
			PTReceiptInvoiceTemplateBundle bundle) throws ExportServiceException {
		return super.toFile(fileURI, bundle.getXSLTFileStream(),
				this.mapDocumentToParamsTree(invoice, bundle), bundle);
	}

	protected void toStream(PTReceiptInvoiceEntity invoice,
			OutputStream targetStream, PTReceiptInvoiceTemplateBundle bundle)
	...
	protected ParamsTree<String, String> mapDocumentToParamsTree(
			PTReceiptInvoiceEntity invoice, PTReceiptInvoiceTemplateBundle bundle) {
	...
	public <T extends ExportServiceRequest> void export(T request,
		...
		if (!(request instanceof PTReceiptInvoicePDFExportRequest)) {
			throw new ExportServiceException("Cannot handle request of type "
					+ request.getClass().getCanonicalName());
		}
		PTReceiptInvoicePDFExportRequest exportRequest = (PTReceiptInvoicePDFExportRequest) request;
		UID docUid = exportRequest.getDocumentUID();

		try {
			PTReceiptInvoiceEntity receiptInvoice = (PTReceiptInvoiceEntity) this.daoPTReceiptInvoice
					.get(docUid);
