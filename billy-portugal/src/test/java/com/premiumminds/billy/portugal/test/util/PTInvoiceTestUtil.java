...
public class PTInvoiceTestUtil {
  ...
  public PTInvoiceEntity getInvoiceEntity(SourceBilling billing) {
    ...
    switch (billing) {
    case M:
      invoice = (PTInvoiceEntity) this
          .getManualInvoiceBuilder(business.getBusinessEntity(), billing).build();
      break;
    case P:
    default:
      invoice = (PTInvoiceEntity) this.getInvoiceBuilder(business.getBusinessEntity(), billing)
          .build();
      break;
    }
  ...
  public PTInvoice.Builder getInvoiceBuilder(PTBusinessEntity business, SourceBilling billing) {
    ...
    return invoiceBuilder.setBilled(PTInvoiceTestUtil.BILLED)
        .setCancelled(PTInvoiceTestUtil.CANCELLED).setSelfBilled(PTInvoiceTestUtil.SELFBILL)
        .setDate(new Date()).setSourceId(PTInvoiceTestUtil.SOURCE_ID).setCustomerUID(customerUID)
        .setSourceBilling(billing).setBusinessUID(business.getUID())
        .addPayment(payment.getPaymentBuilder());
  }

  public PTInvoice.ManualBuilder getManualInvoiceBuilder(PTBusinessEntity business,
  ...
  public PTInvoiceEntity getDiferentRegionsInvoice() {
    ...
    invoiceBuilder.setBilled(PTInvoiceTestUtil.BILLED).setCancelled(PTInvoiceTestUtil.CANCELLED)
        ...
        .setSourceId(PTInvoiceTestUtil.SOURCE_ID).setCustomerUID(customerUID)
        .setSourceBilling(SourceBilling.P).setBusinessUID(business.getBusinessEntity().getUID())
        .addPayment(payment.getPaymentBuilder());
