...
public class TestConcurrentIssuing extends PTDocumentAbstractTest {
  ...
  public void testConcurrentIssuing() throws InterruptedException, ExecutionException {
    ...
    PTBusinessEntity businessEntity1 = new PTBusinessTestUtil(injector).getBusinessEntity(B1);
    String B2 = "Business 2";
    PTBusinessEntity businessEntity2 = new PTBusinessTestUtil(injector).getBusinessEntity(B2);

    List<Future<?>> results1 = test
        .runThreads(new TestRunner(PTAbstractTest.injector, "A", businessEntity1));

    List<PTInvoice> invoices1 = this.executeThreads(results1);

    List<Future<?>> results2 = test
        .runThreads(new TestRunner(PTAbstractTest.injector, "A", businessEntity2));
  ...
  public void testDifferenteBusinessAndSeries() throws InterruptedException, ExecutionException {
    ...
    List<Future<?>> results2 = test
        .runThreads(new TestRunner(PTAbstractTest.injector, "B", businessEntity2));
  ...
  public void testMultipleSeriesIssuing() throws InterruptedException, ExecutionException {
    ...
    List<Future<?>> results1 = test
        .runThreads(new TestRunner(PTAbstractTest.injector, "A", businessEntity1));
    List<Future<?>> results2 = test
        .runThreads(new TestRunner(PTAbstractTest.injector, "B", businessEntity1));
    List<Future<?>> results3 = test
        .runThreads(new TestRunner(PTAbstractTest.injector, "C", businessEntity1));
