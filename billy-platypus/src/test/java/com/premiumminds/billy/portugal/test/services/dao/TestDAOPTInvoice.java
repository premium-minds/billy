...
public class TestDAOPTInvoice extends PTPersistencyAbstractTest {
	...
	public void testLastInvoiceNumber() {
		Integer lastNumber = 20;
		String originalSeries = invoiceEntity.getSeries();

		PTInvoiceEntity newInvoice = invoiceUtil.getInvoiceEntity(
				originalSeries, "lastInvoice uid", lastNumber,
				"lastInvoiceEntry uid");
		dao.create(newInvoice);
		PTInvoiceEntity resultInvoice = dao
				.getLatestInvoiceFromSeries(originalSeries);

		assertEquals(resultInvoice.getSeriesNumber(), lastNumber);
		assertEquals(originalSeries, newInvoice.getSeries());
	}
