...
public class TestPTInvoicePDFExportHandler {
	...
	public void testPDFcreation() throws NoSuchAlgorithmException, ExportServiceException {
		PTInvoiceTemplateBundle bundle = 
				new PTInvoiceTemplateBundle(LOGO_PATH, getClass().getResourceAsStream(XSL_PATH), PDF_PATH, BUSINESS_EMAIL, BUSINESS_PHONE, BUSINESS_FAX, SOFTWARE_CERTIFICATE_NUMBER);
		PTInvoicePDFExportHandler handler = new PTInvoicePDFExportHandler();
		/*File file = */handler.toFile(generatePTInvoice(PaymentMechanism.CASH), bundle);
	...
	private PTInvoiceEntity generatePTInvoice(PaymentMechanism paymentMechanism) {
		...
		CustomerEntity customer = new CustomerEntity("António Joaquim da Costa Pascoal"
				...
				, null
				, false);
		customer.setUUID(UUID.fromString(config.get(Config.Key.Customer.Generic.UUID)));
