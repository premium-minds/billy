...
//		public static final String INVOICE_TOTAL = "totalPrice";
	...
	public File toFile(IPTCreditNoteEntity creditNote, PTCreditNoteTemplateBundle bundle) throws ExportServiceException {
		return super.toFile(bundle.getXSLTFileStream(), this.mapDocumentToParamsTree(creditNote, bundle), bundle);
	}

	protected void toStream(IPTCreditNoteEntity creditNote, OutputStream targetStream, PTCreditNoteTemplateBundle bundle) throws ExportServiceException {
	...
	protected ParamsTree<String, String> mapDocumentToParamsTree(
			IPTCreditNoteEntity creditNote, PTCreditNoteTemplateBundle bundle) {
		...
		params.getRoot().addChild(ParamKeys.CN_EMISSION_DATE
				, date.format(creditNote.getIssueDate()));
		...
		businessAddress.addChild(ParamKeys.BUSINESS_ADDRESS_COUNTRY
				, creditNote.getBusiness().getAddress().getCountry());
		businessAddress.addChild(ParamKeys.BUSINESS_ADDRESS_LINE1
				, creditNote.getBusiness().getAddress().getAddressLine1());
		...
		customer.addChild(ParamKeys.CUSTOMER_FINANCIAL_ID
				, (creditNote.getCustomer().getUUID().equals(config.getUUID(Config.Key.Customer.Generic.UUID)) ? bundle.getGenericCustomer() : creditNote.getCustomer().getFinancialID()));
		if(creditNote.getCustomer().getBillingAddress() != null) {
			...
			customerAddress.addChild(ParamKeys.CUSTOMER_BILLING_ADDRESS_COUNTRY
					, creditNote.getCustomer().getBillingAddress().getCountry());
			customerAddress.addChild(ParamKeys.CUSTOMER_BILLING_ADDRESS_LINE1
					, creditNote.getCustomer().getBillingAddress().getAddressLine1());
		...
		for(IPTFinancialDocumentEntryEntity entry : creditNote.getDocumentEntries()) {
			...
			entryNode.addChild(ParamKeys.INVOICE).addChild(ParamKeys.INVOICE_ID, entry.getReferencedDocument().getDocumentNumber());
		}
		...
		params.getRoot().addChild(ParamKeys.CN_TOTAL_BEFORE_TAX
				, creditNote.getNetTotal().setScale(2, RoundingMode.HALF_UP).toPlainString());
		params.getRoot().addChild(ParamKeys.CN_TOTAL_TAX
				, creditNote.getTaxTotal().setScale(2, RoundingMode.HALF_UP).toPlainString());
		params.getRoot().addChild(ParamKeys.CN_TOTAL
				, creditNote.getGrossTotal().setScale(2, RoundingMode.HALF_UP).toPlainString());
		return params;
	}
	...
	public <T extends ExportServiceRequest> void export(T request,
		...
		try {
			IPTCreditNoteEntity creditNote= daoPTCreditNote.get(docUid.getValue());
