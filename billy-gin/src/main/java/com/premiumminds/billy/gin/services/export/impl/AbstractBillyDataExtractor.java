...
public abstract class AbstractBillyDataExtractor {
	...
	protected CostumerData extractCostumer(Customer costumerEntity) {
		...
		AddressData costumerBillingAddress = new AddressData(costumerAddressEntity.getISOCountry(), costumerAddressEntity.getDetails(), 
				costumerAddressEntity.getCity(), costumerAddressEntity.getRegion(), costumerAddressEntity.getPostalCode());
		return new CostumerData(costumerEntity.getUID(), 
				costumerEntity.getTaxRegistrationNumber(), costumerEntity.getName(), 
				costumerBillingAddress);
	}
	
	protected BusinessData extractBusiness(Business businessEntity) {
		Address businessAddressEntity = businessEntity.getAddress();
		AddressData businessAddress = new AddressData(businessAddressEntity.getISOCountry(), businessAddressEntity.getDetails(), 
				businessAddressEntity.getCity(), businessAddressEntity.getRegion(), businessAddressEntity.getPostalCode());
		Contact contactEntity = businessEntity.getMainContact();
		ContactData businessContact = new ContactData(contactEntity.getTelephone(), contactEntity.getFax(), contactEntity.getEmail());
	...
	protected List<TaxData> extractTaxes(Collection<Tax> taxEntities) {
		...
		for (Tax tax : taxEntities) {
			taxes.add(new TaxData(tax.getUID(), tax.getValue(), tax.getTaxRateType(), tax.getDescription(), tax.getDesignation()));
		}
