...
public class DAOESCreditReceiptImpl extends DAOESGenericInvoiceImpl implements DAOESCreditReceipt {
  ...
  public List<ESCreditReceipt> findByReferencedDocument(UID uidCompany, UID uidInvoice) {
    ...
    JPASubQuery entQ = new JPASubQuery().from(entry).where(
        toDSL(entry.reference, QJPAESGenericInvoiceEntity.class).uid.in(invQ.list(recepit.uid)));

    return (List<ESCreditReceipt>) (List<?>) createQuery().from(creditReceipt)
        .where(
            toDSL(creditReceipt.business, QJPAESBusinessEntity.class).uid.eq(uidCompany.toString())
                .and(toDSL(creditReceipt.entries.any(), QJPAESCreditReceiptEntryEntity.class).uid
