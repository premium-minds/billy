...
public class TestESInvoiceBuilder extends ESAbstractTest {
	...
	private static final String ES_INVOICE_YML = AbstractTest.YML_CONFIGS_DIR
			+ "ESInvoice.yml";
	...
	@Test
	public void doTest() {
		MockESInvoiceEntity mock = this.createMockEntity(
				MockESInvoiceEntity.class, TestESInvoiceBuilder.ES_INVOICE_YML);
		...
		Mockito.when(this.getInstance(DAOESInvoice.class).getEntityInstance())
				.thenReturn(new MockESInvoiceEntity());

		MockESInvoiceEntryEntity entryMock = this.createMockEntity(
				MockESInvoiceEntryEntity.class,
				TestESInvoiceBuilder.ES_INVOICE_ENTRY_YML);

		Mockito.when(
				this.getInstance(DAOESInvoiceEntry.class).get(
						Matchers.any(UID.class))).thenReturn(entryMock);

		mock.getEntries().add(entryMock);

		ArrayList<ESInvoiceEntry> entries = (ArrayList<ESInvoiceEntry>) mock
				.getEntries();

		ESInvoice.Builder builder = this.getInstance(ESInvoice.Builder.class);

		ESInvoiceEntry.Builder entry = this
				.getMock(ESInvoiceEntry.Builder.class);

		MockESPaymentEntity mockPayment = this.createMockEntity(
				MockESPaymentEntity.class,
				TestESInvoiceBuilder.ES_PAYMENT_YML);
