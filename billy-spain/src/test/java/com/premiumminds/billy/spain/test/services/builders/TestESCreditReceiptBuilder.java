...
public class TestESCreditReceiptBuilder extends ESAbstractTest {
	...
	private static final String ES_CUSTOMER_YML = AbstractTest.YML_CONFIGS_DIR
			+ "ESCustomer.yml";
	...
	@Test
	public void doTest() {
		MockESCreditReceiptEntity mock = this.createMockEntity(
				MockESCreditReceiptEntity.class,
		...
		Mockito.when(
				this.getInstance(DAOESReceipt.class).getEntityInstance())
				.thenReturn(new MockESReceiptEntity());

		Mockito.when(
				this.getInstance(DAOESCreditReceipt.class).getEntityInstance())
		...
		Mockito.when(
				this.getInstance(DAOESCreditReceiptEntry.class).get(
		...
		ESCreditReceipt.Builder builder = this
				.getInstance(ESCreditReceipt.Builder.class);

		ESCreditReceiptEntry.Builder entry1 = this
				.getMock(ESCreditReceiptEntry.Builder.class);
		...
		Assert.assertEquals(creditReceipt.getEntries().size(), mock.getEntries()
				.size());
		...
		Assert.assertEquals(mock.getGeneralLedgerDate(),
				creditReceipt.getGeneralLedgerDate());
