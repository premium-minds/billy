...
public class TestESSupplierBuilder extends ESAbstractTest {
  ...
  @Test
  public void doTest() {
    MockESSupplierEntity mockSupplier = this.createMockEntity(MockESSupplierEntity.class,
        TestESSupplierBuilder.ESSUPPLIER_YML);

    Mockito.when(this.getInstance(DAOESSupplier.class).getEntityInstance())
        .thenReturn(new MockESSupplierEntity());

    ESAddress.Builder mockMainAddressBuilder = this.getMock(ESAddress.Builder.class);
    Mockito.when(mockMainAddressBuilder.build())
        .thenReturn((ESAddressEntity) mockSupplier.getMainAddress());
    ...
    Mockito.when(mockBillingAddressBuilder.build())
        .thenReturn((ESAddressEntity) mockSupplier.getBillingAddress());
    ...
    builder.addAddress(mockMainAddressBuilder).addAddress(mockShippingAddressBuilder)
        ...
        .setTaxRegistrationNumber(mockSupplier.getTaxRegistrationNumber(), ES_COUNTRY_CODE)
    ...
    Assert.assertEquals(mockSupplier.getBankAccounts().size(),
