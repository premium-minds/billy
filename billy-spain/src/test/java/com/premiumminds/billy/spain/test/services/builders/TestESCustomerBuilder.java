...
public class TestESCustomerBuilder extends ESAbstractTest {
  ...
  @Test
  public void doTest() {
    MockESCustomerEntity mockCustomer = this.createMockEntity(MockESCustomerEntity.class,
        TestESCustomerBuilder.ESCUSTOMER_YML);

    Mockito.when(this.getInstance(DAOESCustomer.class).getEntityInstance())
        .thenReturn(new MockESCustomerEntity());
    ...
    Mockito.when(mockMainAddressBuilder.build())
        .thenReturn((ESAddressEntity) mockCustomer.getMainAddress());
    ...
    Mockito.when(mockBillingAddressBuilder.build())
        .thenReturn((ESAddressEntity) mockCustomer.getBillingAddress());
    ...
    Mockito.when(mockContactBuilder1.build())
        .thenReturn((ESContactEntity) mockCustomer.getContacts().get(0));

    ESContact.Builder mockContactBuilder2 = this.getMock(ESContact.Builder.class);
    Mockito.when(mockContactBuilder2.build())
        .thenReturn((ESContactEntity) mockCustomer.getContacts().get(1));
    ...
    builder.addAddress(mockMainAddressBuilder, true).addContact(mockMainContactBuilder)
        ...
        .setHasSelfBillingAgreement(mockCustomer.hasSelfBillingAgreement())
        .setName(mockCustomer.getName()).setShippingAddress(mockShippingAddressBuilder)
        .setTaxRegistrationNumber(mockCustomer.getTaxRegistrationNumber(), ES_COUNTRY_CODE)
