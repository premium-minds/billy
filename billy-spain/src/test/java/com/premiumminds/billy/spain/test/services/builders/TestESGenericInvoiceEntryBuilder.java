...
public class TestESGenericInvoiceEntryBuilder extends ESAbstractTest {
	...
	@Test
	public void doTest() {
		MockESGenericInvoiceEntryEntity mock = this.createMockEntity(
		...
		Mockito.when(
				this.getInstance(DAOESGenericInvoiceEntry.class)
						.getEntityInstance()).thenReturn(
				new MockESGenericInvoiceEntryEntity());

		MockESGenericInvoiceEntity mockInvoice = this.createMockEntity(
				MockESGenericInvoiceEntity.class,
				TestESGenericInvoiceEntryBuilder.ES_GENERIC_INVOICE_YML);

		Mockito.when(
				this.getInstance(DAOESGenericInvoice.class).get(
		...
		Mockito.when(
				this.getInstance(DAOESRegionContext.class).isSubContext(
						Matchers.any(ESRegionContext.class),
						Matchers.any(ESRegionContext.class))).thenReturn(true);

		mock.getDocumentReferences().add(mockInvoice);

		ESGenericInvoiceEntry.Builder builder = this
				.getInstance(ESGenericInvoiceEntry.Builder.class);
