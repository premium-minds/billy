...
public class ESCreditReceiptTestUtil {
  ...
  public ESCreditReceipt.Builder getCreditReceiptBuilder(ESReceiptEntity reference) {
    ...
    return creditReceiptBuilder.setBilled(ESCreditReceiptTestUtil.BILLED)
        ...
        .setSelfBilled(ESCreditReceiptTestUtil.SELFBILL).setDate(new Date())
        .setSourceId(ESCreditReceiptTestUtil.SOURCEID).addEntry(creditReceiptEntryBuilder)
        .setBusinessUID(reference.getBusiness().getUID()).addPayment(payment.getPaymentBuilder());
  }
}
