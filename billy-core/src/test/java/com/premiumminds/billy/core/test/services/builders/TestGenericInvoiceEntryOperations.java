...
public class TestGenericInvoiceEntryOperations extends AbstractTest {
  ...
  public void testOperations() {
    ...
    Assert.assertTrue(entry.getAmountWithoutTax().compareTo(mock.getAmountWithoutTax()) == 0);

    Assert.assertTrue(entry.getAmountWithoutTax()
        .compareTo(mock.getUnitAmountWithoutTax().multiply(this.qnt, this.mc)) == 0);

    Assert.assertTrue(entry.getAmountWithoutTax()
        .compareTo((mock.getAmountWithTax().subtract(mock.getTaxAmount(), this.mc))) == 0);

    Assert.assertTrue(entry.getAmountWithTax()
        .compareTo(mock.getAmountWithTax().setScale(7, this.mc.getRoundingMode())) == 0);

    Assert.assertTrue(entry.getAmountWithTax()
        .compareTo(mock.getUnitAmountWithTax().multiply(this.qnt, this.mc)) == 0);

    Assert.assertTrue(entry.getAmountWithTax()
        .compareTo((mock.getTaxAmount().add(mock.getAmountWithoutTax(), this.mc))) == 0);

    Assert.assertTrue(entry.getTaxAmount().compareTo(mock.getTaxAmount()) == 0);

    Assert.assertTrue(entry.getTaxAmount()
        .compareTo((mock.getAmountWithTax().subtract(mock.getAmountWithoutTax()))) == 0);

    Assert.assertTrue(
        entry.getTaxAmount().compareTo(mock.getUnitTaxAmount().multiply(this.qnt, this.mc)) == 0);

    Assert.assertTrue(entry.getUnitAmountWithTax().compareTo(mock.getUnitAmountWithTax()) == 0);

    Assert.assertTrue(entry.getUnitAmountWithTax()
        .compareTo((mock.getUnitAmountWithoutTax().add(
            mock.getUnitAmountWithoutTax().multiply(new BigDecimal("0.23"), this.mc),
            this.mc))) == 0);

    Assert
        .assertTrue(entry.getUnitAmountWithoutTax().compareTo(mock.getUnitAmountWithoutTax()) == 0);

    Assert.assertTrue(entry.getUnitAmountWithoutTax()
        .compareTo((mock.getUnitAmountWithTax().subtract(mock.getUnitTaxAmount(), this.mc))) == 0);

    Assert.assertTrue(entry.getUnitTaxAmount().compareTo(mock.getUnitTaxAmount()) == 0);

    Assert.assertTrue(entry.getUnitTaxAmount().compareTo(
        (mock.getUnitAmountWithTax().subtract(mock.getUnitAmountWithoutTax(), this.mc))) == 0);
    ...
    Assert.assertTrue(entry.getTaxAmount().compareTo(mock.getTaxAmount()) == 0);
    Assert.assertTrue(entry.getAmountWithoutTax().compareTo(mock.getAmountWithoutTax()) == 0);
    Assert.assertTrue(entry.getAmountWithTax().compareTo(mock.getAmountWithTax()) == 0);
  ...
  public MockGenericInvoiceEntryEntity getMockEntryEntity(MockGenericInvoiceEntity invoice,
    ...
    MockGenericInvoiceEntryEntity result = this.createMockEntity(
        MockGenericInvoiceEntryEntity.class, TestGenericInvoiceEntryOperations.ENTRY_YML);
