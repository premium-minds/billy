...
public class TestApplicationBuilder extends AbstractTest {
  ...
  @Test
  public void doTest() {
    MockApplicationEntity mockApplication = this.createMockEntity(MockApplicationEntity.class,
    ...
    builder.addContact(mockContactBuilder1).addContact(mockContactBuilder2)
    ...
    for (int i = 0; i < application.getContacts().size() - 1; i++) {
      ...
      Assert.assertEquals(appContacts.get(i).getName(),
          mockApplication.getContacts().get(i).getName());
    }
