...
private static final String TAX_YML = "src/test/resources/Tax.yml";
	...
	public void doTest() {
		...
		Mockito.when(getInstance(DAOTax.class).getEntityInstance()).thenReturn(new MockTaxEntity());
		
		Tax.Builder builder = getInstance(Tax.Builder.class);
		
		builder.setCode(mockTax.getCode()).setContextUID(mockTax.getContext().getUID()).setCurrency(mockTax.getCurrency()).setDescription(mockTax.getDescription()).setDesignation(mockTax.getDesignation()).setTaxRate(mockTax.getTaxRateType(), mockTax.getValue()).setValidFrom(mockTax.getValidFrom()).setValidTo(mockTax.getValidTo());
	...
	public MockTaxEntity loadFixture(Class<MockTaxEntity> clazz) {
		MockTaxEntity result = (MockTaxEntity) createMockEntityFromYaml(MockTaxEntity.class, TAX_YML);
		
		result.uid = new UID("uid_tax");
		
		MockContextEntity mockContext = new MockContextEntity();
		mockContext.uid = new UID("uid_context");
		Mockito.when(getInstance(DAOContext.class).get(Matchers.any(UID.class))).thenReturn(mockContext);
