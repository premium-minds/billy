...
public class TestShippingPointBuilder extends AbstractTest {
    ...
    @Test
    public void doTest() {
        MockShippingPointEntity mockShippingPoint =
                this.createMockEntity(MockShippingPointEntity.class, TestShippingPointBuilder.SHIPPINGPOINT_YML);

        Mockito.when(this.getInstance(DAOShippingPoint.class).getEntityInstance())
        ...
        builder.setAddress(mockAddressBuilder).setDate(mockShippingPoint.getDate())
                ...
                .setUCR(mockShippingPoint.getUCR()).setWarehouseId(mockShippingPoint.getWarehouseId());
