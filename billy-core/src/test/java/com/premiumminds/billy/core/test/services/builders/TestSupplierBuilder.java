...
private static final String SUPPLIER_YML = "src/test/resources/Supplier.yml";
	...
	public void doTest() {
		...
		Mockito.when(getInstance(DAOSupplier.class).getEntityInstance()).thenReturn(new MockSupplierEntity());
		...
		builder.addAddress(mockAddressBuilder).addContact(mockContactBuilder).setBankAccount(mockBankAccountBuilder).setBillingAddress(mockAddressBuilder).setMainAddress(mockAddressBuilder).setMainContact(mockContactBuilder).setName(mockSupplier.getName()).setSelfBillingAgreement(false).setShippingAddress(mockAddressBuilder).setTaxRegistrationNumber(mockSupplier.getTaxRegistrationNumber());
		
		Supplier supplier = builder.build();
		
		assert(supplier != null);
		assertEquals(mockSupplier.getName(), supplier.getName());
		assertEquals(mockSupplier.getTaxRegistrationNumber(), supplier.getTaxRegistrationNumber());
		
		
	}
	
	public MockSupplierEntity loadFixture(Class<MockSupplierEntity> clazz) {
		MockSupplierEntity result = (MockSupplierEntity) createMockEntityFromYaml(MockSupplierEntity.class, SUPPLIER_YML);
