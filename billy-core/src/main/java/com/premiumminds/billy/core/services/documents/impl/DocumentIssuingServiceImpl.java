...
public class DocumentIssuingServiceImpl implements DocumentIssuingService {
	...
	public DocumentIssuingServiceImpl(){
		handlers = new HashMap<Class<? extends GenericInvoice>, DocumentIssuingHandler>();
	}

	@Override
	public <T extends GenericInvoice> T issue(T document) throws DocumentIssuingException {
		Type[] types = document.getClass().getGenericInterfaces();
		for(Type type : types) {
			if(handlers.containsKey(type)) {
				return handlers.get(type).issue(document);
			}
		}
